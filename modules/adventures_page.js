    ...
    adventures.forEach((element)=>{
      ...
      <a href="detail/?adventure=${element.id}" id="${element.id}">
         <div class="activity-card">
          <img src=${element.image} class="img" alt=${element.name}>
...
function filterByDuration(list, low, high) {
  ...
  let newlist=list.filter(element=>low<=element.duration && element.duration<=high);
  console.log(newlist);
  return newlist;
}
...
function generateFilterPillsAndUpdateDOM(filters) {
  ...
  filters.category.forEach(cat=>{
    catlist.innerHTML+=`<div class="category-filter">${cat}<button type="button" onclick="closebutton(event)" class="btn-close btn-sm catclose" aria-label="Close"></button></div>`;
  });
}
